...
@Component({
  selector: 'app-todo-dms-folder-navigator',
  standalone: true,
  imports: [
    CommonModule,
    MatIconModule,
  ...
  templateUrl: './todo-dms-folder-navigator.component.html',
  styleUrls: ['./todo-dms-folder-navigator.component.scss']
})
export class TodoDmsFolderNavigatorComponent {
  ...
  constructor(
    ...
    private dialog: MatDialogRef<TodoDmsFolderNavigatorComponent>,
  ) {
    if (data) {
  ...
  isCopied(file: ProjectAttachment): boolean {
    return this.todoService.copied.some((f) => f.id === file.id);
  }

  toggleCopy(file: ProjectAttachment) {
    this.todoService.toggleCopy(file);
  }
