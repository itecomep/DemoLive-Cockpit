...
import {
  ...
  UploadResult,
} from "../../../mcv-file/components/mcv-file-upload/mcv-file-upload.component";
...
export class ProjectInwardDialogComponent implements OnInit {
  ...
  private async getContactOptions() {
    this.contactOptions = (
      await firstValueFrom(
        this.contactService.get(this.contactFilter, "", "fullName"),
      )
    ).filter((x) => x.email != undefined);
  }

  filterContacts(property: string): any[] {
    return this.contactOptions.filter((option: Contact) =>
      option
        ? option.name.toLowerCase().includes(property.toLowerCase()) ||
          option.emails.find((x) =>
            x.email.toLowerCase().includes(property.toLowerCase()),
          )
        : false,
    );
  }
  ...
  private async uploadFiles(entity: ProjectInward) {
    ...
    console.log("DIALOG DMS", metadata);
  ...
  async onSubmit() {
    ...
    if (this.isCreateMode) {
      ...
      this.inward = Object.assign(
        this.inward,
        await firstValueFrom(this.projectInwardService.update(this.inward)),
      );
      await this.uploadFiles(this.inward);
