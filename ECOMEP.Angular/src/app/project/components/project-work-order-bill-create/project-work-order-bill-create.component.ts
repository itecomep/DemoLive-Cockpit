...
import { AsyncPipe, CommonModule, CurrencyPipe, DecimalPipe, NgFor } from '@angular/common';
import { BillStage, ProjectBill, ProjectBillPayment } from '../../models/project-bill.model';
...
@Component({
  selector: 'app-project-work-order-bill-create',
  standalone: true,
  imports: [
    CommonModule,
    AsyncPipe,
  ...
  templateUrl: './project-work-order-bill-create.component.html',
  styleUrls: ['./project-work-order-bill-create.component.scss']
})
export class ProjectWorkOrderBillCreateComponent {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<ProjectWorkOrderBillCreateComponent>,
          ...
          if (value) {
            ...
            if (selected) {
              ...
              for (let i = 0; i < selected.length; i++) {
                if (selected[i]) {
                  console.log('selected', this.workOrderStages[i]);
                  this.selectedStages.push(this.workOrderStages[i]);
                }
              }
              this.bill.workPercentage = this.selectedStages.reduce((a, b) => a + b.percentage, 0);
              this.f['billPercentage'].setValue(this.bill.workPercentage);
              this.f['workOrderNo'].setValue(this.workOrder.workOrderNo);
              this.f['workOrderDate'].setValue(this.workOrder.workOrderDate);
            }
          }
        }
      )
  ...
  async onSubmit() {
    ...
    if (this.project && this.workOrder) {
      ...
      this.bill.stages = this.workOrderStages.filter(x => this.selectedStages.find(y => y.id == x.id)).map(x => new BillStage({
        id: x.id,
        title: x.title,
        abbreviation: x.abbreviation,
        percentage: x.percentage,
        amount: Math.round(((this.workOrder.amount * (x.percentage / 100)) + Number.EPSILON) * 100) / 100,
        statusFlag: x.statusFlag,
        orderFlag: x.orderFlag
      }));
      this.bill.workOrderID = this.workOrder.id;
