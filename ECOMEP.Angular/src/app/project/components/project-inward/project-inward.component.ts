...
import { DmsService } from "src/app/project/services/dms.service";
...
export class ProjectInwardComponent {
  ...
  get PROJECT_INWARDS_TYPE_FLAG_INWARDS() {
    return this.projectInwardService.PROJECT_INWARDS_TYPE_FLAG_INWARDS;
  }
  get PROJECT_INWARDS_TYPE_FLAG_PHOTOS() {
    return this.projectInwardService.PROJECT_INWARDS_TYPE_FLAG_PHOTOS;
  }
  get PROJECT_INWARDS_TYPE_FLAG_REPORTS() {
    return this.projectInwardService.PROJECT_INWARDS_TYPE_FLAG_REPORTS;
  }
  ...
  async ngOnInit() {
    ...
    const categoryOptions = this.appSettingService.presets.find(
    ...
    this.blobConfig = {
      container: this.appSettingService?.presets?.find(
  ...
  async getContactOptions() {
    this.contactOptions = (
      await firstValueFrom(
        this.contactService.getEmailOptions(this.contactFilter, "", "fullName"),
      )
    ).filter((x) => x.email != undefined);
  }
  async getProjectInwards() {
    this.inwards = await firstValueFrom(
      this.projectInwardService.get(
        [
  ...
  async onCreate() {
    ...
    if (!this.f["contact"].value || !this.f["contact"].value.id) {
      this.utilityService.showSwalToast(
        "Invalid Contact!",
        "Please select a valid contact and try again!",
        "error",
      );
      return;
    }
    ...
    if (this.inward.attachments.length == 0) {
      this.utilityService.showSweetDialog(
        "No Attachments",
        "Please upload atleast one file and try again.",
        "error",
      );
      return;
    }
    ...
    this.inward.contactID = this.f["contact"].value
      ? this.f["contact"].value.id
      : null;
    this.inward.projectID = this.project.id;
    this.inward.typeFlag = this.typeFlag;

    this.inward = await firstValueFrom(
      this.projectInwardService.create(this.inward),
    );
  ...
  onView(item: ProjectInward) {
    const dialogRef = this.projectInwardService.openDialog(
      ProjectInwardDialogComponent,
      {
  ...
  onExportReport(reportName: string, size: string = "a4") {
    if (this.project) {
      this.projectInwardService.exportReport(
        this.project?.uid,
        reportName,
        size,
      );
  ...
  filterContacts(property: string): any[] {
    return this.contactOptions.filter((option: EmailOption) =>
      option
  ...
  private async uploadFiles(entity: ProjectInward) {
    ...
    if (_createRequests.length != 0) {
      ...
      const metadata = {
        ...
        files: entity.attachments.map((x) => ({
          fileName: x.filename || "",
          blobPath: x.blobPath || "",
          blobUrl: x.url || "",
          fileSize: x.size || 0,
      ...
      console.log("OLD UI DMS", metadata);

      await firstValueFrom(this.dmsService.uploadMetadata(metadata));
    }
  }
  ...
  getFilteredAttachments(
    ...
    return isMedia
      ? attachments.filter(
          (x) =>
            this.mcvfileUtilityService.isImage(x.filename) ||
            this.mcvfileUtilityService.isVideo(x.filename) ||
            this.mcvfileUtilityService.isAudio(x.filename),
        )
      : attachments.filter(
          (x) =>
            !(
