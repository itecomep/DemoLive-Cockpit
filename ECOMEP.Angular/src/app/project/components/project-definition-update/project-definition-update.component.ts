...
import {
  ...
  CountryCode,
  StateCode,
} from "src/app/shared/models/locations";
...
export class ProjectDefinitionUpdateComponent implements OnInit {
  ...
  get isPermissionBillingEdit() {
    return this.projectService.isPermissionBillingEdit;
  }
  ...
  get dialogTitle() {
    return `New ${this.nameOfEntity}`;
  }
  get nameOfEntity() {
    return this.entityApiService.nameOfEntity;
  }
  get isPermissionEdit(): boolean {
    return this.entityApiService.isPermissionEdit;
  }
  get isPermissionDelete() {
    return this.entityApiService.isPermissionDelete;
  }
  get isPermissionList() {
    return this.entityApiService.isPermissionDelete;
  }
  ...
  async ngOnInit() {
    ...
    const _referredContact = this.contactOptions.filter(
      (x) =>
        x.fullName.toLowerCase() ==
        this.currentEntity.referredByContact?.fullName.toLowerCase(),
    );
    if (_referredContact) {
      this.f["referredByContact"].setValue(_referredContact[0], {
        emitEvent: false,
      });
    }

    const _clientContact = this.contactOptions.filter(
      (x) =>
        x.fullName.toLowerCase() ==
        this.currentEntity.clientContact?.fullName.toLowerCase(),
    );
    if (_clientContact) {
      this.f["clientContact"].setValue(_clientContact[0], { emitEvent: false });
    }

    const _groupContact = this.contactOptions.filter(
      (x) => x.id == this.currentEntity.groupContact?.id,
    );
    if (_groupContact) {
    ...
    this.filteredReferrerOptions = this.referredByFC.valueChanges.pipe(
      ...
      map((name) =>
        name
          ? this.filterContacts(name as string)
    ...
    this.filteredGroupCompanyOptions =
      this.groupCompanyContactFC.valueChanges.pipe(
        debounceTime(400),
        distinctUntilChanged(),
        startWith<string | any>(""),
        map((value) =>
  ...
  private buildForm() {
    this.form = this.formBuilder.group({
      ...
      code: new FormControl<any>(null, {
        nonNullable: true,
        validators: [Validators.required],
      }),
      contractStartDate: new FormControl<any>(new Date(), {
        nonNullable: true,
        validators: [Validators.required],
      }),
      contractCompletionDate: new FormControl<any>(new Date(), {
        nonNullable: true,
        validators: [Validators.required],
      }),
      ...
      billingTitle: new FormControl<any>(null, {
        nonNullable: true,
        validators: [Validators.required],
      }),
      ...
      segments: new FormControl<any>(null, {
        nonNullable: true,
        validators: [Validators.required],
      }),
      typologies: new FormControl<any>(null, {
        nonNullable: true,
        validators: [Validators.required],
      }),
    ...
    this.filteredLocationOptions = this.f["location"].valueChanges.pipe(
      ...
      map((name) =>
        name
          ? this._filterLocation(name as string)
          : this.locationOptions.slice(),
      ),
    );
    ...
    this.filteredCityOptions$ = this.f["city"].valueChanges.pipe(
      ...
      distinctUntilChanged(),
      map((value) =>
        typeof value === "string" ? value : (value as CityOption)?.city,
      ),
      map((value: string) =>
    ...
    this.f["city"].valueChanges
      ...
      .subscribe((value: string) => {
        if (value) {
          ...
          const stateInfo = this.stateCodeOptions.find(
            (x) => x.state === option?.state,
          );
    ...
    this.filteredStateOptions$ = this.f["state"].valueChanges.pipe(
      ...
      distinctUntilChanged(),
      map((value) =>
        typeof value === "string" ? value : (value as CityOption)?.state,
      ),
      map((value: string) =>
    ...
    this.filteredCountryOptions$ = this.f["country"].valueChanges.pipe(
      ...
      distinctUntilChanged(),
      map((value) =>
        typeof value === "string" ? value : (value as CountryCode)?.country,
      ),
      map((value: string) =>
    ...
    this.filteredStateCodeOptions$ = this.f["stateCode"].valueChanges.pipe(
      ...
      map((name) =>
        name
          ? this._filterStateCode(name as any)
          : this.stateCodeOptions.slice(),
      ),
    );
  ...
  bindForm() {
    ...
    this.f["segments"].setValue(this.currentEntity.segment?.split(","), {
      emitEvent: false,
    });
    this.f["typologies"].setValue(this.currentEntity.typology?.split(","), {
      emitEvent: false,
    });
    ...
    if (
      this.currentEntity.expectedCompletionDate &&
      this.currentEntity.expectedCompletionDate < new Date()
    ) {
      const date = new Date();
      date.setDate(date.getDate() + 1);
      this.f["expectedCompletionDate"].setValue(date);
    } else {
      this.f["expectedCompletionDate"].setValue(
        this.currentEntity.expectedCompletionDate,
      );
    }
  ...
  private async getExisting(title: string) {
    let filters: ApiFilter[] = [{ key: "title", value: title }];
    const data = await firstValueFrom(this.entityApiService.get(filters));
    this.existing = data.filter(
      (x) =>
        x.companyID == this.currentEntity.companyID &&
        x.id != this.currentEntity.id,
    );
  ...
  private filterState(value?: string) {
    const uniqueStringArray = [
      ...new Set(this.cityOptions.map((x) => x.state)),
    ];
    if (!value) {
      return uniqueStringArray;
    }
    return uniqueStringArray.filter((x) =>
  ...
  private async getContactOptions() {
    if (this.contactOptions.length == 0)
      this.contactOptions = await firstValueFrom(
        this.contactService.getOptions(this.contactFilter, "", "fullName"),
      );
  }
  ...
  private async getLocationOptions() {
    this.cityOptions = await firstValueFrom(
      this.utilityService.getCityOptions(),
    );
    this.stateCodeOptions = await firstValueFrom(
      this.utilityService.getStateGSTINCodes(),
    );
    this.countryCodeOptions = await firstValueFrom(
      this.utilityService.getCountryCodes(),
    );
    const res = await firstValueFrom(
      this.entityApiService.getFieldOptions("Location"),
    );
  ...
  private async getSegmentOptions() {
    if (
      !this.appSettingService.presets ||
      !this.appSettingService.presets.length
    ) {
      await this.appSettingService.loadPresets();
    }
    const _setting = this.appSettingService.presets.find(
      (x) => x.presetKey == this.config.PRESET_PROJECT_SEGMENT_OPTIONS,
    );
    if (_setting) {
      this.segmentOptions = _setting.presetValue
        .split(",")
        .map((x) => x.toUpperCase());
  ...
  private async getTypologyOptions() {
    ...
    const _setting = this.appSettingService.presets.find(
      (x) => x.presetKey == this.config.PRESET_PROJECT_TYPOLOGY_OPTIONS,
    );
    if (_setting) {
      this.typologyOptions = _setting.presetValue
        .split(",")
        .map((x) => x.toUpperCase());
      // this.f["segment"].setValue(this.segmentOptions[0], { emitEvent: false });
    }
  ...
  onSubmit() {
    ...
    this.currentEntity.clientContactID = this.f["clientContact"].value
      ? this.f["clientContact"].value.id
      : null;
    this.currentEntity.groupContactID = this.f["groupContact"].value
      ? this.f["groupContact"].value.id
      : null;
    this.currentEntity.referredByContactID = this.f["referredByContact"].value
      ? this.f["referredByContact"].value.id
      : null;
    this.currentEntity.groupCompanyContactID = this.f["groupCompanyContact"]
      .value
      ? this.f["groupCompanyContact"].value.id
      : null;
    ...
    this.utilityService.showConfirmationDialog(_messageText, async () => {
      ...
      this.currentEntity = await firstValueFrom(
        this.entityApiService.update(this.currentEntity),
      );
      this.f["comment"].reset();
  ...
  onCreateChild() {
    this.utilityService.showConfirmationDialog(
      `Do you want create child project for ${this.currentEntity.title}?`,
      async () => {
        ...
        const _project = await firstValueFrom(
          this.entityApiService.create(_childProject),
        );
        this.entityApiService.refreshList();
  ...
  onDeleteClientContact() {
    this.utilityService.showConfirmationDialog(
      `Do you want to remove  ${this.currentEntity.clientContact?.fullName} ?`,
      async () => {
  ...
  onDeleteReferredContact() {
    this.utilityService.showConfirmationDialog(
      `Do you want to remove  ${this.currentEntity.referredByContact?.fullName} ?`,
      async () => {
        ...
        this.utilityService.showSwalToast(
          "",
          "Referred by contact removed successfully!!",
          "success",
        );
      },
    );
  ...
  onDeleteGroupContact() {
    this.utilityService.showConfirmationDialog(
      `Do you want to remove  ${this.currentEntity.groupContact?.fullName} ?`,
      async () => {
        ...
        this.utilityService.showSwalToast(
          "",
          "Group contact removed successfully!!",
          "success",
        );
      },
    );
  ...
  onDeleteGroupCompanyContact() {
    this.utilityService.showConfirmationDialog(
      `Do you want to remove  ${this.currentEntity.groupCompanyContact?.fullName} ?`,
      async () => {
        ...
        this.utilityService.showSwalToast(
          "",
          "Group company contact removed successfully!!",
          "success",
        );
      },
    );
