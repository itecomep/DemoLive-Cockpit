...
import { RequestTicketListItemComponent } from '../request-ticket-list-item/request-ticket-list-item.component';
...
@Component({
  selector: 'app-request-ticket-paged-list',
  templateUrl: './request-ticket-paged-list.component.html',
  styleUrls: ['./request-ticket-paged-list.component.scss'],
  standalone: true,
  imports: [NgFor, RouterLinkActive, NgClass, RouterLink, RequestTicketListItemComponent, NgIf, MatTooltipModule]
})
export class RequestTicketPagedListComponent implements OnInit, OnDestroy
{
  ...
  isDelayed(item: RequestTicket): boolean
  {
    const due = new Date(item.nextReminderDate);
    const now = new Date();
  ...
  isForDelay(item: RequestTicket): boolean
  {
    const due = new Date(item.nextReminderDate);
    const now = new Date();
    const next = new Date();
