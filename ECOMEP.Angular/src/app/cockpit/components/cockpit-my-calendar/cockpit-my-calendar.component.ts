...
@Component({
  selector: 'app-cockpit-my-calendar',
  standalone: true,
  imports: [FullCalendarModule, MatTooltipModule, FooterComponent, CommonModule, MatFormFieldModule, MatInputModule, ReactiveFormsModule, MatIconModule, MatButtonModule, MatTooltipModule],
  templateUrl: './cockpit-my-calendar.component.html',
  styleUrls: ['./cockpit-my-calendar.component.scss']
})
export class CockpitMyCalendarComponent {
  ...
  private getEvents() {
    ...
    this.getLeaveEvents(eventsFilter);
    this.getMeetingEvents(eventsFilter);
    this.getSiteVisitEvents(eventsFilter);
    this.getTodoEvents(eventsFilter);
    this.getHolidays();
  }
  ...
  private getTodoEvents(eventsFilter: ApiFilter[]) {
    eventsFilter = eventsFilter.filter(x => x.key !== 'contactID');
    this.todoService.get(eventsFilter.concat([
      { key: 'statusFlag', value: '0' },
      // { key: 'statusFlag', value: '1' },
      { key: 'assigneeContactID', value: this.currentContact?.id.toString() || '' }
    ]), this.searchKey).pipe(
      map((items: any[]) => items.map(x => {
