...
export class McvElasticDirective implements OnInit, AfterViewInit, OnDestroy {

  private elasticid: any;
  @Input('Elastic')
  set elasticId(elasticId: any) {
  ...
  set left(left: number) {
    ...
    // this.snapToGrid === "both" || this.snapToGrid === "horizontal"
  ...
  set width(width: number) {
    ...
    // this.snapToGrid === "both" || this.snapToGrid === "horizontal"
    //   ? Math.ceil(width / this.minWidth) * this.minWidth
    //   : width;
  ...
  set top(top: number) {
    ...
    // this.snapToGrid === "both" || this.snapToGrid === "vertical"
    ...
    observables.drags.forEach((event) => {
      ...
      obj.y =
        this.moveDirection === 'both' || this.moveDirection === 'vertical'
          ? event.y
          : 0;
      if (!this.isDisabled && !this.service.isResizing) {
    ...
    observables.drops.forEach((event) => {
      ...
      obj.y =
        this.moveDirection === 'both' || this.moveDirection === 'vertical'
          ? event.y
          : 0;
      if (!this.isDisabled && !this.service.isResizing && this.service.isMoving) {
  ...
  updateElement() {
    if (this.isDisabled) {
      this.renderer.removeClass(this.elasticElement, 'mcv-elastic');
    } else {
      this.renderer.addClass(this.elasticElement, 'mcv-elastic');
    }
  }
