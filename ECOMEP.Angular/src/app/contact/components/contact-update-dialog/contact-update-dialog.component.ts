import { Component, EventEmitter, Inject, inject, Input, NgZone, OnInit, Output, ViewChild } from '@angular/core';
...
@Component({
  selector: 'app-contact-update-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatIconModule,
  ...
  styleUrls: ['./contact-update-dialog.component.scss']
})
export class ContactUpdateDialogComponent implements OnInit {
  ...
  constructor(
    ...
    private dialog: MatDialogRef<ContactUpdateDialogComponent>,
    private contactAssociationService: ContactAssociationApiService,
    @Inject(MAT_DIALOG_DATA) data: any
  ) {
    this.data = data;
    this.currentEntity = this.data.contact;
  }
  ...
  onDelete(): void {
    ...
    const _messageText = `If the person is associated with any project, please remove them from that project first.
           Delete ${this.nameOfEntity}: ${this.currentEntity.name}`;
  ...
  onSubmit() {
    ...
    this.currentEntity = Object.assign(this.currentEntity, this.form.value);
