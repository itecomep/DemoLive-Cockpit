...
@Component({
    selector: 'app-meeting-agenda-task',
    templateUrl: './meeting-agenda-task.component.html',
    styleUrls: ['./meeting-agenda-task.component.scss'],
    standalone: true,
    imports: [NgIf, WftaskTitleBarComponent, MatExpansionModule, MatButtonModule, MeetingAgendaComponent]
})
export class MeetingAgendaTaskComponent 
{
  //Common component properties-------------------------
  currentEntity: MeetingAgenda = new MeetingAgenda();
  nameOfEntity = this.config.NAMEOF_ENTITY_MEETING_AGENDA;
  entityID!: number;
  task: WFTask | any;
  ...
  constructor(private entityService: MeetingAgendaApiService,
  ...
  async getCurrent(id: number)
  {
    ...
    this.meeting = await firstValueFrom(this.meetingService.getById(this.currentEntity.meetingID));
  }
  ...
  async onEditClick()
  {
    ...
    const _agendaFormConfig = {
      ...
      isCreatemode: false,
      meetingID: this.currentEntity.meetingID,
      meeting: this.meeting,
      agenda: this.currentEntity,
      dialogTitle: `Update Agenda | ${this.currentEntity.title}-${this.currentEntity.subtitle}`
    };
    const dialogRef = this.entityService.openDialog(MeetingAgendaFormComponent, _agendaFormConfig);
  ...
  onCreateTodo()
  {
    ...
    let _todo = new Todo({
      ...
      title: this.currentEntity.meetingTitle + '-' + this.currentEntity.title,
