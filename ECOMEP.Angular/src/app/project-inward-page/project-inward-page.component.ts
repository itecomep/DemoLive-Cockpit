...
export class ProjectInwardPageComponent implements OnInit {
  ...
  buildForm() {
    ...
    this.filteredContacts$ = this.f["contact"].valueChanges.pipe(
      ...
      map((name) =>
        name ? this.filterContacts(name) : this.contactOptions.slice(),
      ),
    );
    ...
    const interval = setInterval(() => {
      if (this.appSettingService.presets?.length) {
        const data = this.appSettingService.presets.find(
          (x) =>
            x.presetKey == this.config.PRESET_PROJECT_INWARD_CATEGORY_OPTIONS,
        )?.presetValue;
  ...
  onUpload(uploads: UploadResult[]) {
    ...
    this.inward.attachments = [...this.inward.attachments, ...newAttachments];
  ...
  async onSubmit() {
    ...
    this.inward.typeFlag = 1;

    try {
      this.inward = await firstValueFrom(
        this.inwardService.create(this.inward),
      );

      await this.uploadFiles(this.inward);

      console.log("SAVED SUCCESSFULLY", this.inward);
  ...
  private async uploadFiles(entity: ProjectInward) {
    ...
    if (entity.attachments.length > 0) {
      ...
      console.log("DMS METADATA", metadata);
  ...
  searchProjects(value: string): Observable<any[]> {
    ...
    return this.projectService
      .getPages(0, 1000, filters, value, "code desc")
