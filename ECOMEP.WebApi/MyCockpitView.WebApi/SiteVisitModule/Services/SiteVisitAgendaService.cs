...
public interface ISiteVisitAgendaService : IBaseEntityService<SiteVisitAgenda>
{
    Task<string> GetSiteVisitAgendaHistoryString(int AgendaID);
    Task SendAgendaFollowUpEmails();
}

public class SiteVisitAgendaService : BaseEntityService<SiteVisitAgenda>, ISiteVisitAgendaService
{

    public SiteVisitAgendaService(EntitiesContext db) : base(db)
    {
    }

    public IQueryable<SiteVisitAgenda> Get(IEnumerable<QueryFilter>? Filters = null, string? Search = null, string? Sort = null)
    {

        IQueryable<SiteVisitAgenda> _query = base.Get(Filters).Include(x => x.Attachments);
        ...
        if (Filters != null)
        {
            ...
            {
                var predicate = PredicateBuilder.False<SiteVisitAgenda>();
            ...
            if (Filters.Where(x => x.Key.Equals("SiteVisitContactID", StringComparison.OrdinalIgnoreCase)).Any())
            {
                var predicate = PredicateBuilder.False<SiteVisitAgenda>();
                foreach (var _item in Filters.Where(x => x.Key.Equals("SiteVisitContactID", StringComparison.OrdinalIgnoreCase)))
                {
                    var isNumeric = Convert.ToInt32(_item.Value);

                    predicate = predicate.Or(x => x.SiteVisit.ContactID == isNumeric);
                }
                _query = _query.Include(x => x.SiteVisit).Where(predicate);
            }
            if (Filters.Where(x => x.Key.Equals("projectPartnerContactID", StringComparison.OrdinalIgnoreCase)).Any())
            {
                var _activeProjects = db.Projects.AsNoTracking()
            ...
            if (Filters.Where(x => x.Key.Equals("contactID", StringComparison.OrdinalIgnoreCase)).Any())
            {
                ...
                foreach (var _item in Filters.Where(x => x.Key.Equals("contactID", StringComparison.OrdinalIgnoreCase)))
                {
                    ...
                    predicate = predicate.Or(x => (x.ActionByContactID != null && x.ActionByContactID == isNumeric)
                     || (x.SiteVisit.ContactID == isNumeric)
                     || (x.ProjectID != null
            ...
            if (Filters.Where(x => x.Key.Equals("SiteVisitTitle", StringComparison.OrdinalIgnoreCase)).Any())
            {
                var predicate = PredicateBuilder.False<SiteVisitAgenda>();
                foreach (var _item in Filters.Where(x => x.Key.Equals("SiteVisitTitle", StringComparison.OrdinalIgnoreCase)))
                {
                    predicate = predicate.Or(x => x.SiteVisit.Title.Equals(_item.Value, StringComparison.OrdinalIgnoreCase));
                }
                _query = _query.Include(x => x.SiteVisit).Where(predicate);
            }
            if (Filters.Where(x => x.Key.Equals("SiteVisitstatusFlag", StringComparison.OrdinalIgnoreCase)).Any())
            {
                var predicate = PredicateBuilder.False<SiteVisitAgenda>();
                foreach (var _item in Filters.Where(x => x.Key.Equals("SiteVisitstatusFlag", StringComparison.OrdinalIgnoreCase)))
                {
                    var isNumeric = Convert.ToInt32(_item.Value);

                    predicate = predicate.Or(x => x.SiteVisit.StatusFlag == isNumeric);
                }
                _query = _query.Include(x => x.SiteVisit).Where(predicate);
            }
            ...
            if (Filters.Where(x => x.Key.Equals("usersOnly", StringComparison.OrdinalIgnoreCase)).Any())
            {
                ...
                //|| (_activeUserContactIDs.Any(c => c == x.SiteVisit.ContactID))
                );
    ...
    public async Task<SiteVisitAgenda> GetById(int Id)
    {
        var query = await Get()
    ...
    public async Task<SiteVisitAgenda> GetById(Guid Id)
    {
        var query = await Get()
          .Include(x => x.Attachments)
    ...
    public async Task<string> GetSiteVisitAgendaHistoryString(int AgendaID)
    {

        var _agenda = await db.SiteVisitAgendas.AsNoTracking().Where(x => !x.IsDeleted)
            .Include(x => x.SiteVisit.Contact)
        ...
        {
            _commentHistory.AppendLine("[" + ClockTools.GetIST(_agenda.SiteVisit.StartDate).ToString("dd MMM yyyy HH:mm")
              + " "
              + _agenda.SiteVisit.Contact.FullName
              + " "
              + (_agenda.SiteVisit.TypeFlag == 1 ? " C-NOTE " :
              (_agenda.SiteVisit.TypeFlag == 2 ? " INSPECTION " : " SiteVisit")) + "] ");
        }
        ...
        //if (_agenda.SiteVisit.TypeFlag == 1)
        //{
    ...
    public async Task<int> Create(SiteVisitAgenda Entity)
    {
        var SiteVisit = await db.SiteVisits.AsNoTracking()
            .SingleOrDefaultAsync(x => x.ID == Entity.SiteVisitID);

        if (SiteVisit == null) throw new EntityServiceException($"{nameof(SiteVisit)} not found!");

        Entity.SiteVisitDate = SiteVisit.StartDate;
        Entity.SiteVisitTitle = SiteVisit.Title;
        Entity.ProjectID = SiteVisit.ProjectID;
        Entity.TypeFlag = SiteVisit.TypeFlag;

        return await base.Create(Entity);
    }

    public async Task Update(SiteVisitAgenda UpdatedEntity)
    {

        var _originalEntity = await db.SiteVisitAgendas.AsNoTracking().SingleOrDefaultAsync(x => x.ID == UpdatedEntity.ID);

        if (_originalEntity == null) throw new EntityServiceException("SiteVisit agenda not found!");
        ...
        // if (_originalEntity.TypeFlag != McvConstant.SITE_VISIT_TYPEFLAG_INSPECTION)
        // {
        ...
        var _SiteVisit = await db.SiteVisits.AsNoTracking().Where(x => x.ID == UpdatedEntity.SiteVisitID).SingleOrDefaultAsync();
        if (_SiteVisit == null) throw new EntityServiceException("SiteVisit not found for this agenda!");
        UpdatedEntity.ProjectID = _SiteVisit.ProjectID;
        UpdatedEntity.SiteVisitDate = _SiteVisit.StartDate;
        UpdatedEntity.SiteVisitTitle = _SiteVisit.Title;
        UpdatedEntity.TypeFlag = _SiteVisit.TypeFlag;
        //await RecordVersion(Entity.ID);
        UpdatedEntity.NotDiscussed = false;
        if (_SiteVisit.TypeFlag == McvConstant.SITE_VISIT_TYPEFLAG_SITE_VISIT && (UpdatedEntity.Comment == null || UpdatedEntity.Comment.Trim() == String.Empty))
        {
        ...
        if (UpdatedEntity.StatusFlag == McvConstant.SITE_VISIT_AGENDA_STATUSFLAG_RESOLVED)
        {
        ...
        if (_SiteVisit.TypeFlag == McvConstant.SITE_VISIT_TYPEFLAG_SITE_VISIT
            && _SiteVisit.StatusFlag >= McvConstant.SITE_VISIT_STATUSFLAG_SENT) //after Sent
        {
              ...
              )  //Any change after Minutes sent
            {
                ...
                UpdatedEntity.UpdateFrom = "POST-SiteVisit";
                if (!UpdatedEntity.SendUpdate)
                {
                    ...
                    UpdatedEntity.PreviousHistory = await GetSiteVisitAgendaHistoryString(UpdatedEntity.ID);
                }
    ...
    public async Task Delete(int Id)
    {
        ...
        if (entity.PreviousAgendaID != null)
        {
            ...
            if (_previousAgenda != null)
            {
                ...
                //if (_previousAgenda.StatusFlag == McvConstant.SITE_VISIT_AGENDA_STATUSFLAG_PENDING)
                //    await AssignAgendaTasks(_previousAgenda.ID);

                await Update(_previousAgenda);
            }
        }
        var attachmentService = new BaseAttachmentService<SiteVisitAgendaAttachment>(db);
    ...
    public async Task SendAgendaFollowUpEmails()
    {
        ...
        var _filters = new List<QueryFilter>
                {
                    new QueryFilter { Key = "statusFlag", Value = McvConstant.SITE_VISIT_AGENDA_STATUSFLAG_PENDING.ToString() },
                    ...
                    new QueryFilter { Key = "typeFlag", Value = McvConstant.SITE_VISIT_TYPEFLAG_SITE_VISIT.ToString() },
                                //   new QueryFilter { Key = "typeFlag", Value = McvConstant.SITE_VISIT_TYPEFLAG_CNOTE.ToString() },
                };
        ...
        var _contactGroup = await Get()
            .Where(x => !x.IsReadOnly)
                .Where(x => x.StatusFlag == McvConstant.SITE_VISIT_AGENDA_STATUSFLAG_PENDING && !x.IsForwarded)
         ...
         .Select(x => new {
             ...
             SiteVisitTitle = x.SiteVisitTitle
         })
         .Distinct()
         .ToListAsync();
                ...
                if (_contact != null)
                {
                    ...
                    var _groupAgendas = await Get(_filters)
                         .Where(x => x.DueDate.Value.Date < _reportDate)
                          .Where(x => x.ActionByContactID != null)
                        .Where(x =>
                        x.SiteVisitTitle == group.SiteVisitTitle &&
                    ...
                    if (_groupAgendas.Any())
                    {
                        try
                        {
                            var _SiteVisitID = _groupAgendas.FirstOrDefault().SiteVisitID;
                            var _SiteVisit = await db.SiteVisits.AsNoTracking()
                                .Include(x => x.Contact)
                                .Include(x => x.Attendees)
                                .Where(x => !x.IsReadOnly)
                                .Where(x => x.ID == _SiteVisitID)
                                .SingleOrDefaultAsync();
                            ...
                            var _receiver = _SiteVisit.Attendees.FirstOrDefault(x => x.ContactID == group.ContactID);
                            if (_receiver == null)
                            {
                                _receiver = new SiteVisitAttendee { Name = _contact.FullName, Email = _contact.Emails.FirstOrDefault(c => c.IsPrimary)?.Email };
                            }

                            if (_SiteVisit != null && _SiteVisit.StatusFlag == McvConstant.SITE_VISIT_STATUSFLAG_SENT)
                            {
                                ...
                                var _emailSubject = $"{group.SiteVisitTitle} | {_emailHeading} | {_contact.FullName} | {_reportDate.ToString("dd MMM yyyy")}";
                                ...
                                sb.AppendLine("                </td>");
                                sb.AppendLine("                <td valign=\"top\" style=\"padding-bottom: 5px\">");
                                sb.AppendLine(group.SiteVisitTitle);
                                sb.AppendLine("                </td>");
                                sb.AppendLine("            </tr>");
                                sb.AppendLine("            <tr>");
                                sb.AppendLine("                <td valign=\"top\" width=\"120\" style=\"font-weight:bold;padding-bottom:5px;\">");
                                ...
                                sb.AppendLine("            <tr>");
                                sb.AppendLine("                <td valign=\"top\" width=\"120\" style=\"font-weight:bold;padding-bottom:5px;\">");
                                sb.AppendLine("                    SiteVisit Date:");
                                sb.AppendLine("                </td>");
                                sb.AppendLine("                <td valign=\"top\" style=\"padding-bottom: 5px\">");
                                sb.AppendLine(ClockTools.GetIST(_SiteVisit.StartDate).ToString("dd MMM yyyy HH:mm"));
                                sb.AppendLine("                </td>");
                                sb.AppendLine("            </tr>");
