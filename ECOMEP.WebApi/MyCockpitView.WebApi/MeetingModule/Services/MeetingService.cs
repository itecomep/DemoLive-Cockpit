...
{
    Task ScaffoldPendingAttendees(int MeetingID);
...
public class MeetingService : BaseEntityService<Meeting>, IMeetingService
{
    ...
    public IQueryable<Meeting> Get(IEnumerable<QueryFilter>? Filters = null, string? Search = null, string? Sort = null)
    {
        ...
        if (Filters != null)
        {
            ...
            if (Filters.Where(x => x.Key.Equals("attendeeOrCreatedByContactID", StringComparison.OrdinalIgnoreCase)).Any())
            {
                var predicate = PredicateBuilder.False<Meeting>();
                foreach (var _item in Filters.Where(x => x.Key.Equals("attendeeOrCreatedByContactID", StringComparison.OrdinalIgnoreCase)))
                {
                    var isNumeric = Convert.ToInt32(_item.Value);

                    //predicate = predicate.Or(x => x.Attendees.Any(c => c.ContactID == isNumeric) || x.CreatedByContactID == isNumeric);
                    predicate = predicate.Or(x => x.CreatedByContactID == isNumeric);
                    predicate = predicate.Or(x => x.Attendees.Any(a => a.ContactID == isNumeric));
                }
                _query = _query.Include(x => x.Attendees).Where(predicate);
            }
    ...
    public async Task<Meeting> GetById(int Id)
    {
        var query = await Get()
          ...
          .Include(x => x.Vouchers).ThenInclude(x => x.Attachments)
    ...
    public async Task<int> Create(Meeting Entity)
    {
        ...
        if (Entity.TypeFlag == McvConstant.MEETING_TYPEFLAG_MEETING)
        {
            ...
            Entity.Code = $"{(_project != null ? _project.Code : Entity.Title.Replace(" | ", "-"))}-MEET-{Entity.OrderFlag}.{Entity.Annexure}".ToUpper().Trim();
        }
        else if (Entity.TypeFlag == McvConstant.MEETING_TYPEFLAG_INSPECTION)
        {
            var _lastorder = 0;
            var _annexure = 0;
            var _query = db.Meetings.AsNoTracking().Where(x => !x.IsDeleted)
                .Where(x => !x.IsReadOnly)
                .Where(x => x.TypeFlag == Entity.TypeFlag);
            if (Entity.ProjectID != null)
            {
            ...
            Entity.OrderFlag = _lastorder;
            Entity.Annexure = _annexure;
            Entity.Code = $"{(_project != null ? _project.Code : Entity.Title.Replace(" | ", "-"))}-INSP-{Entity.OrderFlag}.{Entity.Annexure}".ToUpper().Trim();
        }
        else if (Entity.TypeFlag == McvConstant.MEETING_TYPEFLAG_CNOTE)
        {
            ...
            if (_lastSent != null)
            {
                _lastorder = _lastSent.OrderFlag;
                _annexure = _lastSent.Annexure;
            }
            ...
            Entity.Code = $"{(_project != null ? _project.Code : Entity.Title.Replace(" | ", "-"))}-CMNT-{Entity.OrderFlag}.{Entity.Annexure}".ToUpper().Trim();

        }

        Entity.ID = await base.Create(Entity);
        //await AddAttendee(Entity.ID, Entity.ContactID, McvConstant.MEETING_ATTENDEE_TYPEFLAG_TO);
    ...
    private async Task AddAttendee(int meetingID, int contactID, int typeFlag, int? companyID = null)
    {
        ...
        if (contact.Appointments.Where(x => x.StatusFlag == McvConstant.APPOINTMENT_STATUSFLAG_APPOINTED).Any()) //Employee
        {
            foreach (var appointment in contact.Appointments.Where(x => x.StatusFlag == McvConstant.APPOINTMENT_STATUSFLAG_APPOINTED))
            {
                if (contact.Emails != null && regex.IsMatch(contact.Emails.FirstOrDefault(x => x.IsPrimary).Email))
                {
                    ...
                    await meetingAttendeeService.Create(_newAttendee);
                }
    ...
    private async Task<Meeting> GetLastPending(Meeting Entity)
    {
        ...
        if (Entity.TypeFlag == McvConstant.MEETING_TYPEFLAG_CNOTE) //C-note
        {
            return _lastNote;
        }
        else if (Entity.TypeFlag == McvConstant.MEETING_TYPEFLAG_INSPECTION) //Inspection
        {
            return await _query
    ...
    public async Task ScaffoldPendingAgenda(int MeetingID, string Subject, int? ProjectID = null, int? typeFlag = null)
    {
        ...
        var _currentAgendas = await db.MeetingAgendas.Where(x => !x.IsDeleted)
        ...
        if (ProjectID != null)
            _query = _query
                .Where(x => x.Meeting.ProjectID != null && x.Meeting.ProjectID == ProjectID);
        else
        ...
        foreach (var previousPending in _pendingList)
        {
            ...
            var _previousAgendaHistory = await meetingAgendaService.GetMeetingAgendaHistoryString(previousPending.ID);
            ...
            if (_currentAgenda == null)
            {
                ...
                await meetingAgendaService.Create(_currentAgenda);
            }
            else
            {
    ...
    private IEnumerable<MeetingAttachment> GetFilteredAttachments(Meeting _meeting, int typeFlag)
    {
        if (_meeting?.Attachments == null || !_meeting.Attachments.Any())
            return Enumerable.Empty<MeetingAttachment>(); // Always return an empty list instead of null

        return _meeting.Attachments
            .Where(x => x.TypeFlag == typeFlag && !string.IsNullOrWhiteSpace(x.Filename))
            .OrderBy(x => x.Filename) // Optional: Sort alphabetically or by another property
            .ToList();
    }
    ...
    public async Task SendMinutes(int ID)
    {
        ...
        var _creator = _meeting.Attendees.Where(x => x.TypeFlag == 0 && x.OrderFlag == 0).FirstOrDefault();
        if (_creator == null)
        {
        ...
        var _internalAttendee = _meeting.Attendees
                                .Where(x => x.TypeFlag == McvConstant.MEETING_ATTENDEE_INTERNAL)
                                .Select(x => new MeetingEmailContact { Name = x.Name, Email = x.Email, Company = x.Company })
                                .ToList();

        var _externalAttendee = _meeting.Attendees
                                .Where(x => x.TypeFlag == McvConstant.MEETING_ATTENDEE_EXTERNAL)
                                .Select(x => new MeetingEmailContact { Name = x.Name, Email = x.Email, Company = x.Company })
                                .ToList();

        var _contactAttendee = _meeting.Attendees
                                .Where(x => x.TypeFlag == McvConstant.MEETING_ATTENDEE_CONTACT_PERSON)
                                .Select(x => new MeetingEmailContact { Name = x.Name, Email = x.Email, Company = x.Company })
                                .ToList();
        ...
        var _toExternalAttendee = _meeting.Attendees
            .Where(x => x.TypeFlag == 1)
            .Where( x => x.isRecipient == true)
            .Where(x => x.Email != null)
            .Select(x => new MeetingEmailContact { Name = x.Name, Email = x.Email, Company = x.Company })
            .ToList();
        ...
        var _allRecipients = _tos.Concat(_toExternalAttendee).ToList();


        var meetingTitle = _meeting.Title;
        if (_meeting.ProjectID != null)
        {
            var projectService = new ProjectService(db);
            var project = await projectService.Get().SingleOrDefaultAsync(x => x.ID == _meeting.ProjectID);
            if (project != null)
                meetingTitle = project.Title;
        ...
        {
            item.PendingAgendaCount = _meeting.Agendas.Where(x => x.StatusFlag == 0)
        ...
        email = _meeting.Contact.Emails.Where(x => x.IsPrimary).Select(y => y.Email).FirstOrDefault();
        if (email != null)
        {
            senderSignatureEmail = email;
        }

        mobile = _meeting.Contact.Phones.Where(x => x.IsPrimary).Select(y => y.Phone).FirstOrDefault();
        if (mobile != null)
        {
        ...
        var _emailHeading = "MINUTES OF MEETING";

        var _emailSubject = $"{meetingTitle} | {_emailHeading} | {ClockTools.GetIST(_meeting.StartDate).ToString("dd MMM yyyy HH:mm")} | #{_meeting.OrderFlag.ToString("000")}" + (_meeting.Version > 0 ? " | R" + _meeting.Version.ToString() : "");

        //if (_meeting.TypeFlag == McvConstant.MEETING_TYPEFLAG_CNOTE)
        //{
        //    _emailHeading = "COMMUNICATION NOTE";
        //    _emailSubject = $"{meetingTitle} | {_emailHeading} | {ClockTools.GetIST(_meeting.StartDate).ToString("dd MMM yyyy HH:mm")} | #{_meeting.OrderFlag.ToString("000.0")}" + (_meeting.Version > 0 ? " | R" + _meeting.Version.ToString() : "");
        //}
        //else if (_meeting.TypeFlag == McvConstant.MEETING_TYPEFLAG_INSPECTION)
        //{
        //    _emailHeading = "INSPECTION REPORT";
        //    _emailSubject = $"{meetingTitle} | {_emailHeading} | {ClockTools.GetIST(_meeting.StartDate).ToString("dd MMM yyyy HH:mm")} | #{_meeting.OrderFlag.ToString("000")}" + (_meeting.Version > 0 ? " | R" + _meeting.Version.ToString() : "");
        //}

        var _version = $"#{_meeting.OrderFlag.ToString("000")}";

        //if (_meeting.TypeFlag == McvConstant.MEETING_TYPEFLAG_CNOTE)
        //    _version = $"#{_meeting.OrderFlag.ToString("000")}.{_meeting.Annexure.ToString()}";
        //else if (_meeting.TypeFlag == McvConstant.MEETING_TYPEFLAG_INSPECTION)
        //    _version = $"#{_meeting.OrderFlag.ToString("000")}";


        //var _replyParameter = $"mailto:{_meeting.Contact.Email1}?cc={_defaultCCList}&subject=RE:{_emailSubject.ToUpper()}";

        var redirectUrl = await sharedService.GetPresetValue(McvConstant.MEETING_MINUTES_URL_ROOT);
        ...
        sb.AppendLine("                <td valign=\"top\" style=\"padding: 2px;\">");
        sb.AppendLine("        <table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%; border-collapse: collapse;\">");
        foreach (var item in _allRecipients)
        {
            sb.AppendLine("            <tr>");
            sb.AppendLine("                <td >");
        ...
        //                <tr>
        //                    <td valign='top' style='font-weight:bold;'>Internal Attendee:</td>
        //                </tr>
        //                <tr>
        ...
        //foreach (var item in _internalAttendee)
        //{
        //    sb.AppendLine($@"
        //    <tr>
        //        <td>{item.Name} <i>({item.Email})</i></td>
        //     </tr>
        //    ");
        //}
        ...
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        //                <tr>
        //                    <td valign='top' style='font-weight:bold;'>External Attendee:</td>
        //                </tr>
        //                <tr>
        //                    <td valign='top' style='padding: 2px;'>
        //                        <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        ...
        //foreach (var item in _externalAttendee)
        //{
        //    sb.AppendLine($@"
        //    <tr>
        //        <td>{item.Name} <i>({item.Email})</i></td>
        ...
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        //                <tr>
        //                    <td valign='top' style='font-weight:bold;'>Contact Person:</td>
        //                </tr>
        //                <tr>
        //                    <td valign='top' style='padding: 2px;'>
        //                        <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        ...
        //foreach (var item in _contactAttendee)
        //{
        //    sb.AppendLine($@"
        //    <tr>
        //        <td>{item.Name} <i>({item?.Email})</i></td>
        ...
        //Meeting Attachments
        //var attachments = GetFilteredAttachments(_meeting, 0); // 0 = typeFlag(meeting attachment)
        //if (attachments != null && attachments.Any())
        //{
        //    sb.AppendLine($@"
        //    <div style='border-bottom: 1px solid #cccccc; margin-top: 15px; margin-bottom: 15px;'></div>
        //    <h4 style='margin: 0 0 8px 0; font-weight: bold; color: #333;'>Meeting Attachments</h4>
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        //        <tr>
        //            <td valign='top'>
        //            Kindly click the below links to download:
        //            </td>
        //        </tr>
        //    </table>
        //    ");
        ...
        //    {
        //        sb.AppendLine($@"
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse; margin-bottom:5px;'>
        //        <tr style='display:flex; align-items:center;'>
        //            <td style='width:40px; font-weight:bold;'>{index:D2}</td>
        //            <td style='padding-left:8px;'>
        //            <a href='{attachment.Url}' style='text-decoration:none; color:#0078d4;'>{attachment.Filename}</a>
        //            </td>
        //        </tr>
        //    </table>");
        //        index++;
        ...
        //Report Attachments
        //var reportAttachments = GetFilteredAttachments(_meeting, 1); // 1 = typeFlag (Report)
        //if (reportAttachments != null && reportAttachments.Any())
        //{
        //    sb.AppendLine($@"
        //    <div style='border-bottom: 1px solid #cccccc; margin-top: 15px; margin-bottom: 15px;'></div>
        //    <h4 style='margin: 0 0 8px 0; font-weight: bold; color: #333;'>Report Attachments</h4>
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        //        <tr>
        //            <td valign='top'>
        //            Kindly click the below links to download:
        ...
        //Photos Attachments
        //var photosAttachments = GetFilteredAttachments(_meeting, 2); // 2 = typeFlag (Photos)
        //if (photosAttachments != null && photosAttachments.Any())
        //{
        //    sb.AppendLine($@"
        //    <div style='border-bottom: 1px solid #cccccc; margin-top: 15px; margin-bottom: 15px;'></div>
        //    <h4 style='margin: 0 0 8px 0; font-weight: bold; color: #333;'>Photos Attachments</h4>
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        //        <tr>
        //            <td valign='top'>
        //            Kindly click the below links to download:
        ...
        //    int index = 1;
        //    foreach (var attachment in photosAttachments)
        //    {
        //        sb.AppendLine($@"
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse; margin-bottom:5px;'>
        //        <tr style='display:flex; align-items:center;'>
        ...
        //Drawing Attachments
        //var drawingAttachments = GetFilteredAttachments(_meeting, 3); // 3 = typeFlag (Photos)
        //if (drawingAttachments != null && drawingAttachments.Any())
        //{
        //    sb.AppendLine($@"
        //    <div style='border-bottom: 1px solid #cccccc; margin-top: 15px; margin-bottom: 15px;'></div>
        //    <h4 style='margin: 0 0 8px 0; font-weight: bold; color: #333;'>Drawing Attachments</h4>
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse;'>
        //        <tr>
        //            <td valign='top'>
        //            Kindly click the below links to download:
        ...
        //    int index = 1;
        //    foreach (var attachment in drawingAttachments)
        //    {
        //        sb.AppendLine($@"
        //    <table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse: collapse; margin-bottom:5px;'>
        //        <tr style='display:flex; align-items:center;'>
        ...
        foreach (var obj in _allRecipients)
        ...
        _meeting.FinalizedOn = DateTime.UtcNow;
        _meeting.StatusFlag = 1;

        await base.Update(_meeting);
    ...
    public async Task<bool> IsMeetingEditable(int ID, int ContactID)
    {
        ...
        var _meeting = await db.Meetings.AsNoTracking().Where(x => !x.IsDeleted)
            .Where(x => x.ID == ID)
            .SingleOrDefaultAsync();

        if (_meeting == null) throw new EntityServiceException("Meeting not found!");

        if (_meeting.IsReadOnly) return false;
        ...
        if (_meeting.StatusFlag == McvConstant.MEETING_STATUSFLAG_SENT)
        {
            ...
            {
                var _nextMeeting = await db.Meetings.AsNoTracking().Where(x => !x.IsDeleted)
                      .Where(x => x.ID != _meeting.ID)
                      .Where(x => !x.IsReadOnly)
                       .Where(x => x.Title == _meeting.Title)
                    .Where(x => x.StartDate > _meeting.StartDate)
    ...
    public async Task<bool> IsDelayed(int ID)
    {
        ...
        var _task = await db.WFTasks.AsNoTracking()
            ...
            && x.EntityID == _meeting.ID && x.StatusFlag == McvConstant.WFTASK_STATUSFLAG_PENDING)
            .OrderBy(x => x.DueDate)
            .FirstOrDefaultAsync();
    ...
    public async Task<ReportDefinition> GetMinutesReport(string reportSize, Guid uid, string sort = null)
    {
        ...
        //                        .Where(x => x.ProjectID == _meeting.ProjectID)
        //                        .ToListAsync();
        //}
        ...
        if (!_meeting.IsReadOnly && _meeting.StatusFlag != McvConstant.MEETING_STATUSFLAG_SENT)
            reportTitle = "Agenda List";
        else
        {
            if (_meeting.TypeFlag == McvConstant.MEETING_TYPEFLAG_INSPECTION)
                reportTitle = "Inspection Report";
            else if (_meeting.TypeFlag == McvConstant.MEETING_TYPEFLAG_CNOTE)
                reportTitle = "Communication Note";
        }

        var _meetingDate = ClockTools.GetIST(_meeting.StartDate).ToString("dd MMM yyyy HH:mm");
        var _reportProperties = new List<ReportProperties>
                {
                    ...
                    new ReportProperties() { PropertyName = "Title", PropertyValue =_meeting.Title.ToString() },
                    new ReportProperties() { PropertyName = "Number", PropertyValue = $"#{_meeting.OrderFlag.ToString("000")}" },
                    new ReportProperties() { PropertyName = "StartDate", PropertyValue = ClockTools.GetIST(_meeting.StartDate).ToString("dd MMM yyyy HH:mm") },
                                        new ReportProperties() { PropertyName = "EndDate", PropertyValue = ClockTools.GetIST(_meeting.EndDate).ToString("HH:mm") },
                                         new ReportProperties() { PropertyName = "Code", PropertyValue = _meeting.Code },
                    new ReportProperties() { PropertyName = "PreparedBy", PropertyValue = _meeting.Contact.FullName.ToString() },
                     new ReportProperties() { PropertyName = "PreparedOn", PropertyValue = ClockTools.GetISTNow().ToString("dd MMM yyyy HH:mm") },
                    new ReportProperties() { PropertyName = "Location", PropertyValue = _meeting.Location },

                };

        var today = DateTime.UtcNow;
        var agendas = _meeting.Agendas.Where(x => !x.IsDeleted);
        if (sort != null)
        {
        ...
        var attendees = _meeting.Attendees.Select(x => new
        {
            Name = x.Name,
            Email = x.Email,
            Company = x.Company,
            TypeFlag = x.TypeFlag,
            AgendaCount = _meeting.Agendas.Where(m => m.ActionByContactID != null && m.ActionByContactID == x.ContactID).Count(),
        });


        var _images = _meeting.Agendas
            .Where(x => !x.IsDeleted)
        ...
        var _reportPath = $"MeetingMinutes-{reportSize}.rdlc";
        ...
        var _reportDef = new ReportDefinition()
        {
            ...
            Filename = $"MeetingMinutes-{_meeting.Code}-{ClockTools.GetISTNow().ToString("yyMMddHHmm")}",
            RenderType = "PDF"
        };
