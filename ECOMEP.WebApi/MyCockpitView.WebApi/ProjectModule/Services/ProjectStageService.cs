...
public interface IProjectStageService : IBaseEntityService<ProjectStage>
{
}

public class ProjectStageService : BaseEntityService<ProjectStage>, IProjectStageService
{
    public ProjectStageService(EntitiesContext db) : base(db) { }

    public IQueryable<ProjectStage> Get(IEnumerable<QueryFilter>? Filters = null, string? Search = null, string? Sort = null)
    {
       
            IQueryable<ProjectStage> _query = base.Get(Filters);
            ...
            if (Filters != null)
            {
                ...
                {
                    var predicate = PredicateBuilder.False<ProjectStage>();
            ...
            if (Filters.Where(x => x.Key.Equals("ParentID", StringComparison.OrdinalIgnoreCase)).Any())
            {
                ...
                foreach (var _item in Filters.Where(x => x.Key.Equals("ParentID", StringComparison.OrdinalIgnoreCase)))
                {
                    ...
                    predicate = predicate.Or(x => x.ParentID == isNumeric);
                }
                _query = _query.Where(predicate);
            }
        }
    ...
    public async Task<ProjectStage> GetById(int Id)
    {
    ...
    public async Task<ProjectStage> GetById(Guid Id)
    {
