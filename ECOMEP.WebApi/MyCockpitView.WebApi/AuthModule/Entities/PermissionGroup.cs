...
public class PermissionGroup : BaseEntity
{
    ...
    [Column("RoleCodes")]
    public string? _roleCodes { get; set; } = string.Empty;
    ...
    public List<string> RoleCodes
    {
        get
        {
            return !string.IsNullOrEmpty(_roleCodes) ?
                _roleCodes.Split(',').ToList() :
                new List<string>();
        }
        set
        {
            _roleCodes = string.Join(",", value);
        }
    }
...
.HasDefaultValue(false);
        ...
        //builder.HasIndex(e => e.RoleCodes);
